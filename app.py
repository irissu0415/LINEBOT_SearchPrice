...
def historySQL(text):
    ...
    for row in query_data:
        row = list(row)
        p_id = row.pop(2)
        row.append("http://essentialprice.online/shop-details/" + f"{p_id}")
        rows.append(row)
...
def imgPredict(img_name):
    ...
    # 載入訓練好的model
    model_pred= tf.lite.Interpreter('./ResNetV2.tflite')
    ...
    brand = {0: '義美全脂鮮乳', 1: '林鳳營全脂鮮乳', 2: '乳香世家鮮乳', 3: '福樂一番鮮特極鮮乳', 4: '光泉全脂鮮乳', 
